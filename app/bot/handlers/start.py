...
async def show_main_menu(
    ...
    await message.answer(
        ...
        "⚡️Nortic — это сервис для комфортного и защищённого подключения к сети.\n\n"
        "С ним вы можете повысить стабильность соединения, защитить личные данные и получить удобный доступ через персональный ключ.\n\n"
        "🚀 Оформите подписку и начните пользоваться сервисом всего за пару минут.\n\n"
        "👇 Выберите действие:",
...
@router.message(F.text.startswith("🎁 Пробный период"))
