...
async def activate_trial_subscription(message: Message, session: AsyncSession, user):
    ...
    try:
        ...
        if str(e) == "NO_ACTIVE_SERVER":
            await message.answer("Нет активного VPN-сервера. Сначала добавьте сервер в базу.")
            return
    ...
    await message.answer(
        ...
        "Ниже отправляю ваш ключ отдельным сообщением для удобного копирования."
    )
...
async def show_main_menu(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer(
        ...
        "Nortic помогает быстро подключиться к VPN и получить персональный ключ без лишних шагов.\n\n"
        "Выберите действие:",
        ...
        if referrer and referrer.id != user.id and user.referred_by_user_id is None:
            ...
            if not existing_referral:
                ...
                await message.answer(
                    ...
                    "После первой оплаченной подписки пригласивший вас пользователь получит прогресс к скидке."
                )
